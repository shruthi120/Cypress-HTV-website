describe("Testing Home tab", () => {
    ...
    it('Checking navbar links are visible', () => {
        ...
        cy.get('.mr-auto > :nth-child(1) > .nav-link')
        .click()
    ...
    it('Validate whether Watch Now - Carousel 1 & 2 are Clickable ', () => {
        const tem=[];
        for(var i=1;i<3;i++){
        cy.get('.btn.btn-lg.vt-btn.vt-btn-primary').should('have.length',20 ).eq(i).click({force:true})
        cy.url()
    ...
    it('validate whether Watch Now - Carousel 3 is Clickable', () => {
        ...
        .wait(1000)
        cy.get('.btn.btn-lg.vt-btn.vt-btn-primary').should('have.length',20 ).eq(5).click({force:true})
        cy.url()
        cy.location('href').then((urlText) => {
            const temp = urlText
            cy.log(temp);
     ...
     it('validate whether Watch Now - Carousel 4 is Clickable', () => {
        ...
        cy.get('.app-logo').click()
        .wait(1000)
        cy.get('.btn.btn-lg.vt-btn.vt-btn-primary').should('have.length',20 ).eq(8).click({force:true})
        cy.url()
        cy.location('href').then((urlText) => {
            const temp = urlText
            cy.log(temp);
    ...
    it('validate whether Buy Now - Carousel 5 is Clickable', () => {
        ...
        .wait(1000)
        //cy.visit('https://testhtvfun.ventunotech.com/')
        cy.get('.btn.btn-lg.vt-btn.vt-btn-primary').should('have.length',20 ).eq(11).click({force:true})
        cy.url()
        cy.location('href').then((urlText) => {
            const temp = urlText
            cy.log(temp);
    ...
    it('Checking Widgets Titles are visible', () => {
         ...
         cy.get(':nth-child(7) > .widget-title > h2', { timeout: 10000 }).invoke('text').then((urlText) => {
             ...
             cy.request({
                 ...
                 .then((response) => {
                     var jsonData = response.body;
                     var Navbar = jsonData.response.data;
                     expect(Navbar.widgets[7].data.title).to.equal(temp)
                 })
         })
     })
     ...
     it('Checking H-Family firt 2 Widgets Cards size', () => {
        ...
        const tem=[];
         for(var i=1;i<3;i++){
        cy.get('#vhl-3 > .carousel-inner > .carousel-item > :nth-child('+i+') > .aspect-ratio-box > .img-wrpr > .card-img-top', { timeout: 20000 }).invoke('attr', 'src').then((urlText) => {
            const temp = urlText.toString();
            cy.log(temp)
             tem.push(temp);
            })
 ...
 /*it('Checking Widget Card indicator - H-Family icon nxt & previous are visible', () => {
     ...
     it('Checking Container is visible', () => {
         cy.get('#aswift_0', { timeout: 20000 }).should('exist');
        ...
        cy.get('.carousel-item-next > .v-card > .aspect-ratio-box > .img-wrpr > .card-img-top', { timeout: 10000 }).invoke('attr', 'src').then((urlText) => {
            ...
            cy.request({
                ...
                .then((response) => {
                    var jsonData = response.body;
                    var Navbar = jsonData.response.data;
                    expect(Navbar.widgets[7].data.cards[4].data.video_thumbnails["16x9"][320]).to.equal(temp)
                })
        })
    })
         ...
         cy.get(':nth-child(7) > .carousel-lg > .carousel-inner > .active > .v-card > .card-body > .card-primary-text', { timeout: 10000 }).invoke('text').then((urlText) => {
             ...
             cy.request({
                 ...
                 .then((response) => {
                     var jsonData = response.body;
                     var Navbar = jsonData.response.data;
                     expect(Navbar.widgets[7].data.cards[4].data.video_name).to.equal(temp)
                 })
         })
     })
     it('Checking Music & Beyond Video 5 is clickable', () => {
        ...
        cy.location('href').then((urlText) => {
            ...
            cy.request({
                ...
                .then((response) => {
                    var jsonData = response.body;
                    var Navbar = jsonData.response.data.widgets[7].data;
                    cy.expect(Navbar.cards[4].data.video_url.url).to.equal(temp);
                })
        })
    })
    ...
    it('Checking Podcast - Video titles are visible', () => {
        const tem=[];
        for(var i=1;i<5;i++){
         cy.get(':nth-child(10) > .carousel-lg > .carousel-inner > .active > :nth-child('+i+') > .card-body > .card-primary-text > .video-name', { timeout: 10000 }).invoke('text').then((urlText) => {
             const temp = urlText.toString();
             cy.log(temp);
             tem.push(temp);
        })
        ...
        cy.get('.carousel-item-next > .v-card > .aspect-ratio-box > .img-wrpr > .card-img-top', { timeout: 10000 }).invoke('attr', 'src').then((urlText) => {
            ...
            cy.request({
                ...
                .then((response) => {
                    var jsonData = response.body;
                    var Navbar = jsonData.response.data;
                    expect(Navbar.widgets[10].data.cards[4].data.video_thumbnails["16x9"][150]).to.equal(temp)
                })
        })
    })
        ...
        cy.get(':nth-child(10) > .carousel-lg > .carousel-inner > .active > .v-card > .card-body > .card-primary-text > .video-name > a', { timeout: 20000 }).invoke('text').then((urlText) => {
            ...
            cy.request({
                ...
                .then((response) => {
                    var jsonData = response.body;
                    var Navbar = jsonData.response.data;
                    expect(Navbar.widgets[10].data.cards[4].data.video_name).to.equal(temp)
                })
        })
    })
    it('Checking Podcast Video 5 is clickable', () => {
        ...
        cy.location('href').then((urlText) => {
            ...
            cy.request({
                ...
                .then((response) => {
                    var jsonData = response.body;
                    var Navbar = jsonData.response.data.widgets[10].data;
                    cy.expect(Navbar.cards[4].data.video_url.url).to.equal(temp);
                })
        })
    })
    ...
    it('Checking Watch Now - Carousel 1 & 2 are visible', () => {
        for(var i=2;i<4;i++){
        cy.get('.btn.btn-lg.vt-btn.vt-btn-primary').should('have.length',20 ).eq(i).invoke('attr','title').then((urlText) => {
            const temp = urlText.toString();
            cy.log(temp);
        })
    }
    for(var j=2;j<4;j++){
        cy.get('.btn.btn-lg.vt-btn.vt-btn-primary').should('have.length',20 ).eq(i).invoke('text').then((urlText) => {
            const temp = urlText.toString();
            cy.log(temp); 
    })
    }
    ...
    it('Checking Watch Now - Carousel 3 is visible', () => {
        cy.get('.btn.btn-lg.vt-btn.vt-btn-primary').should('have.length',20 ).eq(5).invoke('attr','title').then((urlText) => {
            const temp = urlText.toString();
            cy.log(temp);
        })
        cy.get('.btn.btn-lg.vt-btn.vt-btn-primary').should('have.length',20 ).eq(5).invoke('text').then((urlText) => {
            const temp = urlText.toString();
            cy.log(temp);
        })
    })
    it('Checking Watch Now - Carousel 4 is visible', () => {
        cy.get('.btn.btn-lg.vt-btn.vt-btn-primary').should('have.length',20 ).eq(8).invoke('attr','title').then((urlText) => {
            const temp = urlText.toString();
            cy.log(temp);
        })
        cy.get('.btn.btn-lg.vt-btn.vt-btn-primary').should('have.length',20 ).eq(8).invoke('text').then((urlText) => {
            const temp = urlText.toString();
            cy.log(temp);
        })
    })
    it('Checking Buy Now - Carousel 5 is visible', () => {
        cy.get('.btn.btn-lg.vt-btn.vt-btn-primary').should('have.length',20 ).eq(11).invoke('attr','title').then((urlText) => {
            const temp = urlText.toString();
            cy.log(temp);
        })
        cy.get('.btn.btn-lg.vt-btn.vt-btn-primary').should('have.length',20 ).eq(11).invoke('text').then((urlText) => {
            const temp = urlText.toString();
            cy.log(temp);
        })
    })
